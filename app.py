...
def init_state():
    ...
    st.session_state.setdefault("before_components", [])
    st.session_state.setdefault("after_components", [])
    st.session_state.setdefault("selected_component_components", None)
    st.session_state.setdefault("selected_component_before_components", None)
    st.session_state.setdefault("selected_component_after_components", None)
...
def component_card_label(item, index):
    connection = " to GND" if item["kind"].startswith("Shunt") else ""
    symbol = "L" if item["kind"].endswith("L") else "C"
    return f"{index + 1}. {symbol}{connection}\n\n{item['value']:g} {item['unit']}"


def clickable_component_editor(state_key, heading, key_prefix):
    components = st.session_state[state_key]
    selected_key = f"selected_component_{state_key}"

    st.markdown(f"#### {heading}")
    add1, add2, add3, add4 = st.columns([1.25, 1, 1, 1])
    kind = add1.selectbox("Element", TYPES, key=f"{key_prefix}_add_kind")
    value = add2.number_input(
        "Value", min_value=0.1,
        value=2.2 if kind.endswith("L") else 1.0,
        step=0.1, format="%.1f", key=f"{key_prefix}_add_value",
    )
    units = ["nH", "uH"] if kind.endswith("L") else ["pF", "nF"]
    unit = add3.selectbox("Unit", units, key=f"{key_prefix}_add_unit")
    add4.write("")
    add4.write("")
    if add4.button("Add element", type="primary", use_container_width=True, key=f"{key_prefix}_add"):
        components.append({
            "kind": kind, "value": value, "unit": unit,
            "value_si": value * SCALE[unit],
        })
        st.session_state[selected_key] = len(components) - 1
        st.rerun()

    if not components:
        st.caption("No elements added.")
        return

    card_columns = st.columns(len(components))
    for index, item in enumerate(components):
        selected = st.session_state[selected_key] == index
        if card_columns[index].button(
            ("✓ " if selected else "") + component_card_label(item, index),
            key=f"{key_prefix}_card_{index}",
    ...
    if selected_index is None or selected_index >= len(components):
        return
    item = components[selected_index]
    st.markdown(f"**Edit element {selected_index + 1}**")
    edit1, edit2, edit3 = st.columns(3)
    edit_kind = edit1.selectbox(
        "Type", TYPES, TYPES.index(item["kind"]),
        key=f"{key_prefix}_edit_kind_{selected_index}",
    )
    edit_units = ["nH", "uH"] if edit_kind.endswith("L") else ["pF", "nF"]
    current_unit = item["unit"] if item["unit"] in edit_units else edit_units[0]
    edit_value = edit2.number_input(
        "Edit value", min_value=0.1, value=float(item["value"]),
        step=0.1, format="%.1f", key=f"{key_prefix}_edit_value_{selected_index}",
    )
    edit_unit = edit3.selectbox(
        "Edit unit", edit_units, edit_units.index(current_unit),
        key=f"{key_prefix}_edit_unit_{selected_index}_{edit_kind}",
    )
    b1, b2, b3, b4, b5 = st.columns(5)
    if b1.button("Save", type="primary", use_container_width=True, key=f"{key_prefix}_save"):
        components[selected_index] = {
            "kind": edit_kind, "value": edit_value, "unit": edit_unit,
            "value_si": edit_value * SCALE[edit_unit],
        }
        st.rerun()
    if b2.button("Move left", disabled=selected_index == 0, use_container_width=True, key=f"{key_prefix}_left"):
        components[selected_index - 1], components[selected_index] = components[selected_index], components[selected_index - 1]
        st.session_state[selected_key] = selected_index - 1
        st.rerun()
    if b3.button("Move right", disabled=selected_index == len(components) - 1, use_container_width=True, key=f"{key_prefix}_right"):
        components[selected_index + 1], components[selected_index] = components[selected_index], components[selected_index + 1]
        st.session_state[selected_key] = selected_index + 1
        st.rerun()
    if b4.button("Delete", use_container_width=True, key=f"{key_prefix}_delete"):
        components.pop(selected_index)
        st.session_state[selected_key] = min(selected_index, len(components) - 1) if components else None
        st.rerun()
    if b5.button("Clear", use_container_width=True, key=f"{key_prefix}_clear"):
        components.clear()
        st.session_state[selected_key] = None
        st.rerun()


def network_text(components):
    if not components:
        return "[ No matching elements ]"
    return " → ".join(
        f"[{item['kind']} {item['value']:g} {item['unit']}]"
        for item in components
    )


def draw_touchstone_schematic(filename, before_components, after_components, input_port, output_port, z0):
    st.markdown("#### Complete matching schematic")
    st.code(
        f"Source {z0:g} Ω → {network_text(before_components)} → "
        f"Port {input_port + 1}  [{filename}]  Port {output_port + 1} → "
        f"{network_text(after_components)} → Load {z0:g} Ω",
        language=None,
    )
    st.caption(
        "Before-SnP elements are applied at the selected input port. "
        "After-SnP elements are applied at the selected output port. "
        "The element order is left to right in each section."
    )


def component_editor():
    clickable_component_editor("components", "LC network elements", "standalone")


init_state()
st.title("Streamlit Smith Chart Tool")
st.caption("Version 10.0 - Clickable schematics and matching before/after SnP")
...
with st.sidebar:
    ...
    if mode == "Touchstone matching":
        st.markdown("**Matching connection ports**")
        input_port_number = st.selectbox(
            "Before SnP port", list(range(1, ports + 1)), index=0,
            help="Port where the matching network before the SnP device is connected.",
        )
        default_output = 1 if ports == 1 else 2
        output_port_number = st.selectbox(
            "After SnP port", list(range(1, ports + 1)), index=default_output - 1,
            help="Port where the matching network after the SnP device is connected.",
        )
        input_port = input_port_number - 1
        output_port = output_port_number - 1
    else:
        input_port, output_port = 0, 1
    st.header("4. Markers")
...
if mode == "Standalone LC network":
    ...
    draw_touchstone_schematic(
        network.name,
        st.session_state.before_components,
        st.session_state.after_components,
        input_port,
        output_port,
        z0,
    )
    before_tab, after_tab = st.tabs([
        f"Before SnP · Port {input_port + 1}",
        f"After SnP · Port {output_port + 1}",
    ])
    with before_tab:
        clickable_component_editor(
            "before_components", "Matching elements before SnP", "before_snp"
        )
    with after_tab:
        clickable_component_editor(
            "after_components", "Matching elements after SnP", "after_snp"
        )
    draw_touchstone_schematic(
        network.name,
        st.session_state.before_components,
...
try:
    if mode == "Standalone LC network":
        ...
        calculated_s = original_s.copy()
        if st.session_state.before_components:
            calculated_s = touchstone_matching(
                calculated_s, f_hz, st.session_state.before_components, input_port, z0
            )
        if st.session_state.after_components:
            calculated_s = touchstone_matching(
                calculated_s, f_hz, st.session_state.after_components, output_port, z0
            )
        datasets = [("Original", original_s)]
        if st.session_state.before_components or st.session_state.after_components:
